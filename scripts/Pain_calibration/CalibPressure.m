...
%   Original script for thermal pain:
...
function [abort,varargout]=CalibPressure(P,O)
    ...
    if P.startSection<2
        ...
        preexPainful = 1; % then we start with the conservative assumption that the top preexposure pressure was experienced as painful
    end
    ...
    if P.startSection<3        
        ...
        fprintf('\nReady FIRST THRESHOLD at %1.1f kPa.\n',P.awiszus.mu); 
...
% Here it's just the trigger definitions, but will include more eventually
    function [P,O] = SetParameters(P,O)
        ...
        if strcmp(P.env.hostname,'stimpc1')
            ...
            else % note: without arduino, this is NOT necessary on stimpc setup because there is no separate SCR recording device, just spike; therefore, do it with heatOnsetTHE
...
%         end    
        ...
        if P.devices.arduino        
            ...
            %if exist(dev,'variable'); UseCPAR('Kill',dev); end % close any existing arduino com port connection
            UseCPAR('Init',P.com.arduino); % initialize arduino/CPAR
            %UseCPAR('Init',P.com.thermoino,P.com.thermoinoBaud,P.pain.bT,P.pain.rS); % returns handle of serial object
        end
        ...
        P.awiszus.X   = P.awiszus_range; % pressure range (kPA)
        P.awiszus.mu  = P.awiszus_start; % starting value, based on assumed population mean (arm); may be overridden after preexposure check
        P.awiszus.sd  = P.awiszus_sd; % assumed sd of threshold (population level) - average of 1.6 and 1.8 kPa sd for left and right arm from Polianskis et al., 2001
        P.awiszus.sp  = P.awiszus_sp; % assumed spread of threshold (individual level); we started at 0.8 - no empirical information on this for pressure pain

        if ~O.debug.toggle
            % for Preexposure (Section 1)
            P.pain.preExposure = P.preExposure_intensities; % vector of intensities used for preexposure, intended to lead to binary decision "were any of these painful y/n"
            ...
            % for Preexposure (Section 1)
            P.pain.preExposure = P.preExposure_intensities;
...
%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        if ~O.debug.toggleVisual
            ...
            if section == 1
                ...
                if strcmp(P.language,'de')            
                    ...
                    [P.display.screenRes.width, upperEight]=DrawFormattedText(P.display.w, [dstr 'Druckereizen, die leicht schmerzhaft sein können.'], 'center', upperEight+P.lineheight, P.style.white);
            ...
            elseif section == 2
                ...
                if strcmp(P.language,'de')
                    ...
                    [P.display.screenRes.width, upperEight]=DrawFormattedText(P.display.w, ' ', 'center', upperEight+P.lineheight, P.style.white);
                    [P.display.screenRes.width, upperEight]=DrawFormattedText(P.display.w, 'Sie werden konstante Druckereize erhalten.', 'center', upperEight+P.lineheight, P.style.white);
                    ...
                    [P.display.screenRes.width, upperEight]=DrawFormattedText(P.display.w, ['(mindestens) LEICHT DRUCKHAFT ' keyPainful ' war.'], 'center', upperEight+P.lineheight, P.style.white);
                    [P.display.screenRes.width, upperEight]=DrawFormattedText(P.display.w, ' ', 'center', upperEight+P.lineheight, P.style.white);
                    %[P.display.screenRes.width, upperEight]=DrawFormattedText(P.display.w, 'Wir melden uns gleich, falls Sie noch Fragen haben,', 'center', upperEight+P.lineheight, P.style.white);
                    %[P.display.screenRes.width, upperEight]=DrawFormattedText(P.display.w, 'danach geht es los!', 'center', upperEight+P.lineheight, P.style.white);
                ...
                elseif strcmp(P.language,'en')
                    ...
                    [P.display.screenRes.width, upperEight]=DrawFormattedText(P.display.w, ' ', 'center', upperEight+P.lineheight, P.style.white);
                    [P.display.screenRes.width, upperEight]=DrawFormattedText(P.display.w, 'You will receive constant pressure stimuli.', 'center', upperEight+P.lineheight, P.style.white);
            ...
            elseif section == 5 % added to cover the validation part ("step 5")
                ...
                elseif strcmp(P.language,'en')
                    [P.display.screenRes.width, upperEight]=DrawFormattedText(P.display.w, 'Continuing shortly...', 'center', upperEight, P.style.white);
                    [P.display.screenRes.width, upperEight]=DrawFormattedText(P.display.w, ' ', 'center', upperEight+P.lineheight, P.style.white);
                    [P.display.screenRes.width, upperEight]=DrawFormattedText(P.display.w, 'You will receive constant pressure stimuli via the cuff.', 'center', upperEight+P.lineheight, P.style.white);
                    [P.display.screenRes.width, upperEight]=DrawFormattedText(P.display.w, ['Please rate each stimulus using ' keyMoreLessPainful], 'center', upperEight+P.lineheight, P.style.white);
                    [P.display.screenRes.width, upperEight]=DrawFormattedText(P.display.w, ['and confirm with ' keyConfirm '.'], 'center', upperEight+P.lineheight, P.style.white);
                    [P.display.screenRes.width, upperEight]=DrawFormattedText(P.display.w, ' ', 'center', upperEight+P.lineheight, P.style.white);
                    [P.display.screenRes.width, upperEight]=DrawFormattedText(P.display.w, 'Commencing shortly!', 'center', upperEight+P.lineheight, P.style.white);            
            ...
            elseif section == 6 % added to announce thermode (re)placement
                ...
                if strcmp(P.language,'de')            
                    [P.display.screenRes.width, upperEight]=DrawFormattedText(P.display.w, 'Die Versuchsleitung wird nun die Manschette umsetzen,', 'center', upperEight, P.style.white);
                ...
                elseif strcmp(P.language,'en')
                    [P.display.screenRes.width, upperEight]=DrawFormattedText(P.display.w, 'The experimenter will now move the cuff,', 'center', upperEight, P.style.white);
            ...
            elseif section == 7
                ...
                img = imread(sprintf('%s%s_%s.png',P.path.scriptBase,'\GENERIC\img\scaleTranslation1_rgb',lower(P.language))); % 'C:\Users\horing\Documents\MATLAB\projects\P7_EquiNox\
                ...
                if strcmp(P.language,'de')
                    [P.display.screenRes.width, upperEight]=DrawFormattedText(P.display.w, 'Gleich beginnt Teil 2 der Schmerzschwellenmessung.', 'center', upperEight, P.style.white);
                    [P.display.screenRes.width, upperEight]=DrawFormattedText(P.display.w, ' ', 'center', upperEight+P.lineheight, P.style.white);                
                    [P.display.screenRes.width, upperEight]=DrawFormattedText(P.display.w, 'Sie erhalten nun konstante Druckereize um Ihre eben ermittelt Schmerzschwelle herum.', 'center', upperEight+P.lineheight, P.style.white);
                    [P.display.screenRes.width, upperEight]=DrawFormattedText(P.display.w, ['Bitte bewerten Sie jeden Reiz mithilfe ' keyMoreLessPainful], 'center', upperEight+P.lineheight, P.style.white);
                ...
                elseif strcmp(P.language,'en')
                    [P.display.screenRes.width, upperEight]=DrawFormattedText(P.display.w, 'In a moment, part 2 of pain threshold calibration will start.', 'center', upperEight, P.style.white);
                    [P.display.screenRes.width, upperEight]=DrawFormattedText(P.display.w, ' ', 'center', upperEight+P.lineheight, P.style.white);
                    [P.display.screenRes.width, upperEight]=DrawFormattedText(P.display.w, 'You will receive constant pressure stimuli around the threshold that we just determined.', 'center', upperEight+P.lineheight, P.style.white);
                    [P.display.screenRes.width, upperEight]=DrawFormattedText(P.display.w, ['Please rate each stimulus using ' keyMoreLessPainful], 'center', upperEight+P.lineheight, P.style.white);
                    [P.display.screenRes.width, upperEight]=DrawFormattedText(P.display.w, ['and confirm with ' keyConfirm ' on this scale:'], 'center', upperEight+P.lineheight, P.style.white);
                    [P.display.screenRes.width, upperEight]=DrawFormattedText(P.display.w, ' ', 'center', upperEight+P.lineheight, P.style.white);
                    Screen('DrawTexture', P.display.w, tex, [], [P.display.midpoint(1)-475 upperEight+padPx P.display.midpoint(1)+475 upperEight+150+padPx]);
                    [P.display.screenRes.width, upperEight]=DrawFormattedText(P.display.w, ' ', 'center', upperEight+P.lineheight+150, P.style.white);
                    [P.display.screenRes.width, upperEight]=DrawFormattedText(P.display.w, 'The pain threshold is designated as "minimal pain" in the center of the scale.', 'center', upperEight+P.lineheight, P.style.white);
                    [P.display.screenRes.width, upperEight]=DrawFormattedText(P.display.w, ' ', 'center', upperEight+P.lineheight, P.style.white);
                    [P.display.screenRes.width, upperEight]=DrawFormattedText(P.display.w, 'It will start soon!', 'center', upperEight+P.lineheight, P.style.white);
                end

            elseif section == 8

                img = imread(sprintf('%s%s_%s.png',P.path.scriptBase,'\GENERIC\img\scaleTranslation2_rgb',lower(P.language)));
                tex = Screen('MakeTexture', P.display.w, img);            
                padPx = 15;
                ...
                elseif strcmp(P.language,'en')
                    [P.display.screenRes.width, upperEight]=DrawFormattedText(P.display.w, 'The scale you are using to rate the stimuli', 'center', upperEight+P.lineheight, P.style.white);
                    [P.display.screenRes.width, upperEight]=DrawFormattedText(P.display.w, 'will now change as follows:', 'center', upperEight+P.lineheight, P.style.white);
                    [P.display.screenRes.width, upperEight]=DrawFormattedText(P.display.w, ' ', 'center', upperEight+P.lineheight, P.style.white);
                    Screen('DrawTexture', P.display.w, tex, [], [P.display.midpoint(1)-475 upperEight+padPx P.display.midpoint(1)+475 upperEight+343+padPx]);
                    [P.display.screenRes.width, upperEight]=DrawFormattedText(P.display.w, ' ', 'center', upperEight+P.lineheight+343, P.style.white);
                    [P.display.screenRes.width, upperEight]=DrawFormattedText(P.display.w, 'Should a stimulus be below the pain threshold, you would rate it', 'center', upperEight+P.lineheight, P.style.white);
                    [P.display.screenRes.width, upperEight]=DrawFormattedText(P.display.w, 'all the way to the left end of the scale, not in the center anymore.', 'center', upperEight+P.lineheight, P.style.white);
                end
    ...
    function [abort,preexPainful]=Preexposure(P,O,varargin)
        ...
        for i = 1:length(preExpInts)
            ...
            if P.devices.arduino                
                UseCPAR('Set',preExpInts(i),stimDuration); % set stimulus
                ...
                while GetSecs < tStimStart+sum(stimDuration(1:2))
    ...
    function [P] = DoAwiszus(P,O)
        ...
        if painful==-1
            fprintf('No rating provided for pressure %1.1f. Please restart program. Resuming at the current break point not yet implemented.\n',P.painCalibData.AwThr);
            return;
        else
            save([P.out.dir P.out.file], 'P');
            fprintf('\n\nThreshold determined around %1.1f kPa, after %d trials.\nThreshold data and results saved under %s%s.mat.\n',P.painCalibData.AwThr,P.awiszus.N,P.out.dir,P.out.file);        
        end
    ...
    function [P] = GetAwiszus(P)
        ...
        if isempty(painCFiles)
            ...
            if numel(painCFiles)>1
                warning('Multiple calibration data files found. Proceeding with most recent one (%s).',cell2mat(painCFiles(end)))
            end
            painCFiles = painCFiles(end);
            existP = load([P.out.dir cell2mat(painCFiles(end))]); % load existing parameters
            existP = existP.P;
            ...
            P.painCalibData.notes{end+1} = sprintf('Thresholding data imported from %s',[P.out.dir cell2mat(painCFiles(end))]);
        end
    ...
    function [abort]=DisplayStimulus(P,O,nTrial,temp)
        ...
        fprintf('%1.1f kPa stimulus initiated',temp);
        ...
        if P.devices.arduino        
            UseCPAR('Set',temp,stimDuration); % open channel for arduino to ramp up
            UseCPAR('Trigger',P.cpar.stoprule,P.cpar.forcedstart); % start next stimulus
    ...
    function [painful,tThresholdRating]=BinaryRating(P,O,nTrial)
        ...
        if ~O.debug.toggleVisual
            ...
            elseif strcmp(P.language,'en')
                [P.display.screenRes.width, nY]=DrawFormattedText(P.display.w, ['Not painful ' keyNotPainful ' or (at least) slightly painful ' keyPainful '?'], 'center', upperEight, P.style.white); 
            end
    ...
    function P=DetermineSteps(P)
        ...
        fprintf('\nPrepare protocol using %1.1f kPa as threshold with the following specifications:\n--\n',P.painCalibData.AwThr);
        for nTrial = 1:length(P.plateaus.step2Order)
            fprintf('Step %02d: %1.1f kPa\n',nTrial,P.plateaus.step2Order(nTrial));
        end
        fprintf('--\nRepeat, awTT is %1.1f kPa\n',P.painCalibData.AwThr);
    ...
    function [abort,P]=TrialControl(P,O)
        ...
        if P.startSection<4
            if P.toggles.doScaleTransl && P.toggles.doPainOnly
                ...
                step0Order = [P.painCalibData.AwThr P.painCalibData.AwThr+2 P.painCalibData.AwThr-2]; % provide some perithreshold intensities
                fprintf('\n=================================');
                fprintf('\n========SCALE TRANSLATION========\n');
        ...
        if P.startSection<5
            ...
            if any(P.painCalibData.PeriThrStimRatings(P.painCalibData.PeriThrStimType==0)>25) % 25 being an arbitrary threshold
                fprintf('\nSb rated training stimuli at threshold (%1.1f kPa) that should be rated VAS~0\nat',P.painCalibData.AwThr)
    ...
    function [abort]=ApplyStimulus(P,O,trialTemp)
        ...
        if P.devices.arduino        
            UseCPAR('Set',trialTemp,stimDuration); % open channel for arduino to ramp up      
            UseCPAR('Trigger',P.cpar.stoprule,P.cpar.forcedstart); % start next stimulus
    ...
    function P = GetRegressionResults(P)
        ...
        fprintf('To achieve VAS50, use %1.1f% kPa (lin) or %1.1f kPa (sig).\n',painThresholdLin,painThresholdSig);
...
% when skipping parts of the calibration in StartExperimentAt, try to obtain existing calib data
    function P = GetExistingCalibData(P)       
        ...
        elseif numel(painCFiles)>1
            ...
            P.painCalibData.notes{end+1} = sprintf('Perithresholding data imported from %s',[P.out.dir cell2mat(painCFiles(end))]);
        end             
