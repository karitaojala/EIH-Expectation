...
%% CORE VARIABLES: Pressure pain parameters
...
if ~O.debug.toggle
    ...
    % for Testing (Section 2)
    P.presentation.sMinMaxThreshITIs; % seconds between stimuli; will be randomized between two values - to use constant ITI, use two identical values
    P.presentation.sMinMaxThreshCues; % jittered time prior to the stimulus that the white cross turns red; can be [0 0] (to turn off red cross altogether), but else MUST NOT BE LOWER THAN 0.5
    ...
    % for Testing (Section 2)
    P.presentation.sMinMaxThreshITIs = [20 20];
    P.presentation.sMinMaxThreshCues = [0.5 0.5];

end
[P.presentation.thresholdITIs,P.presentation.thresholdCues]=DetermineITIsAndCues(P.test_trials,P.presentation.sMinMaxThreshITIs,P.presentation.sMinMaxThreshCues);
P.presentation.firstThresholdITI;
...
if P.startSection<3
    ...
    for block = 1:P.test_repeats
        ...
        if ~O.debug.toggleVisual
            ...
            [P.display.screenRes.width, ~]=DrawFormattedText(P.display.w, [num2str(P.test_trials) ' stimuli of 60 seconds with breaks in betweeen'], 'center', upperHalf+P.style.lineheight, P.style.white);
            introTextOn = Screen('Flip',P.display.w);
        else
            introTextOn = GetSecs;
        end

        while GetSecs < introTextOn + 5
            [abort]=LoopBreaker(P);
            if abort; break; end
        end
...
while GetSecs < tCrossOn + P.presentation.firstPlateauITI
    [abort]=LoopBreaker(P);
    if abort; break; end
end
