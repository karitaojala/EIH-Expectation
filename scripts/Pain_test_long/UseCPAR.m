...
elseif strcmpi(action,'set')
    ...
    elseif isempty(varargin{2})
        cparClose(dev);
        error('Pressure pain durations (ramp-up and plateau) required.');
    elseif isempty(varargin{3})
        cparClose(dev);
        error('Settings structure (P) required.');
    end
    ...
    try
        ...
        constant_pressure = cparPulse(pressure,plateau_duration,full_onset); % create constant pressure part
...
elseif strcmpi(action,'data')
    ...
    try
        ...
        cparClose(dev);
        error('Getting CPAR data failed.');
    end
end
