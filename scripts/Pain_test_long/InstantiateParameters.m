...
% stimulus parameters
P.preExposure_intensities   = [10 20 30 40 50]; % preexposure pressure intensities (kPa) to test rough ballpark of pain threshold of the participant
...
P.painCalibData.PeriThrN            = 1; % turn on fixation cross for first stimulus
...
P.presentation.sPreexpITI           = 20; % pre-exposure ITI
P.presentation.sPreexpCue           = P.presentation.sStimPlateauPreexp/P.pain.rS+P.presentation.sStimPlateauPreexp; % pre-exposure cue duration (approximate stimulus duration with rise time included)
P.presentation.sMinMaxThreshITIs    = [20 20]; % seconds between stimuli; will be randomized between two values - to use constant ITI, use two identical values
P.presentation.sMinMaxThreshCues    = [0.5 0.5]; % jittered time prior to the stimulus that the white cross turns red; can be [0 0] (to turn off red cross altogether), but else MUST NOT BE LOWER THAN 0.5
P.presentation.firstThresholdITI    = 20;
P.presentation.firstPlateauITI      = 20;
P.presentation.firstPlateauCue      = P.presentation.sStimPlateau/P.pain.rS+P.presentation.sStimPlateau;
